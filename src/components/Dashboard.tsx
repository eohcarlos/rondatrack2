...
export const Dashboard = memo(({ onLogout, onGoHome, companyName }: DashboardProps) => {
  ...
  return (
    <>
      ...
      <div className={`min-h-screen bg-gradient-to-br from-background via-background to-muted/20 transition-all duration-300 overflow-y-auto overflow-x-hidden ${isSidebarOpen ? 'blur-sm pointer-events-none' : ''}`}>
        <DashboardHeader
          companyName={companyName}
          profile={profile}
          isAdmin={isAdmin}
